...
export default function TripCard({
  ...
  return (
    <div
      className={[
        'rounded-lg border bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 px-4 py-4 shadow-sm transition-all cursor-pointer',
        isBlocked ? 'opacity-80' : '',
        selected ? 'border-orange-400 ring-2 ring-orange-100 dark:ring-orange-950/20' : 'border-slate-200 dark:border-slate-850 hover:border-orange-300 dark:hover:border-orange-900/30 hover:shadow-md dark:hover:shadow-orange-950/10',
        className,
      ].join(' ')}
    ...
    >
      <div className="flex flex-col gap-4">
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.2fr_1fr_1.2fr_1fr] xl:items-start">
          {/* Cột Điểm đi */}
          <div>
            <div className="text-[32px] font-extrabold text-slate-900 dark:text-white">{departureTime}</div>
            <div className="mt-2 flex items-start gap-2 text-slate-700 dark:text-slate-350">
              <MapPin className="mt-1 h-4 w-4 text-orange-500" />
              <div>
                <div className="text-[20px] font-semibold dark:text-slate-100">{trip.pickupPoint || trip.from}</div>
                <div className="text-sm text-slate-500 dark:text-slate-400">{formatDate(trip.departDate)}</div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center justify-center pt-2">
            ...
            <div className="mt-2 flex w-full max-w-[200px] items-center">
              ...
              <div className="mx-2 h-[2px] flex-1 bg-slate-300 dark:bg-slate-700" />
          ...
          {/* Cột Điểm đến */}
          <div>
            <div className="text-[32px] font-extrabold text-slate-900 dark:text-white">{arrivalTime}</div>
            <div className="mt-2 flex items-start gap-2 text-slate-700 dark:text-slate-350">
              <MapPin className="mt-1 h-4 w-4 text-orange-500" />
              <div>
                <div className="text-[20px] font-semibold dark:text-slate-100">{trip.dropoffPoint || trip.to}</div>
          ...
          <div className="flex flex-col items-start gap-2 xl:items-end">
            <div className="flex flex-wrap items-center gap-2 text-sm">
              <span className="text-slate-700 dark:text-slate-350">{trip.busType || 'Limousine'}</span>
        ...
        {/* Phần Nút bấm Footer */}
        <div className="flex flex-col gap-2 border-t border-slate-200 dark:border-slate-800 pt-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-wrap items-center gap-4 text-sm text-slate-800 dark:text-slate-300">
          ...
          <button
            ...
            className={[
              ...
              isBlocked ? 'bg-slate-400 dark:bg-slate-800' : selected ? 'bg-slate-800 dark:bg-slate-700' : 'bg-orange-500 hover:bg-orange-600',
            ].join(' ')}
          >
