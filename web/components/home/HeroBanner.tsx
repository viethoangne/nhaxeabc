...
export default function HeroBanner() {
  ...
  return (
    <div className="relative w-full h-[300px] md:h-[500px] lg:h-[750px] overflow-hidden rounded-b-[2.5rem] bg-slate-950 shadow-[0_20px_50px_rgba(0,0,0,0.15)]">
      ...
      <div className="absolute top-6 right-6 md:top-10 md:right-10 z-30">
        <div className="flex items-center gap-6 md:gap-8 rounded-full bg-black/35 backdrop-blur-xl px-6 py-2.5 border border-white/15 shadow-2xl">
          {BANNERS.map((banner, index) => (
            <div
              key={banner.id}
            ...
            >
              ...
              <span className={`text-2xl md:text-3xl font-black italic tracking-wide transition-all duration-300 ${currentIndex === index
                  ? 'text-orange-400 drop-shadow-[0_0_12px_rgba(251,146,60,0.8)] scale-110' // Active state
                  : 'text-white/50 group-hover:text-white/90 group-hover:scale-105' // Normal state
                }`}>
                0{index + 1}
              </span>
            </div>
